...
class TranslationFeatureTest(testing.FeatureExpectationsTestCase):
  ...
  def test_translation_encoded(self):
    ...
    self.assertFeature(
        feature=features.Translation(languages=["en", "zh"],
  ...
  def test_save_load_metadata(self):
    translation_f = features.Translation(
        languages=["zh"],
        encoder=text_encoder.ByteTextEncoder(additional_tokens=["HI"]))
    text = u"HI 你好"
    ids = translation_f["zh"].str2ints(text)
    self.assertEqual(1, ids[0])

    with testing.tmp_dir(self.get_temp_dir()) as data_dir:
      feature_name = "dummy"
      translation_f.save_metadata(data_dir, feature_name)

      new_f = features.Translation([])
      new_f.load_metadata(data_dir, feature_name)
      self.assertEqual(["zh"], translation_f.languages)
      self.assertEqual(ids, translation_f["zh"].str2ints(text))
